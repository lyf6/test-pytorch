...
target_csv = '2019jh-crop.csv'
myFile = open(target_csv,'w',newline='')
writer = csv.writer(myFile)
cls_id = {1: 'farming land',
          2: 'building area',
          3: 'road',
          4: 'pond',
}
...
for img_name in imglist:
    if '2019' in img_name:
        ...
        tif_path = img_name.split('.png')[0] + '.tif'
        tif_path = os.path.join(imgdir, tif_path)
        tif_img = np.array(Image.open(tif_path))
        tmp = tif_img.sum(axis=2)
        w,h = tmp.shape
        size=w*h
        zeros = ((tmp<10*3)).sum()
        if zeros/size<0.2:
