...
img1=np.array(gdal.Open(img1_path).ReadAsArray(), np.int)
img2=np.array(gdal.Open(img2_path).ReadAsArray(), np.int)
...
for val in collect:
    ...
    for re_id in tqdm(range(num_region)):
        ...
        score = perimeter/sub_region_aera
        #print(centroid)
        region_id = label[centroid[0],centroid[1]]-1
        #print(region_id)
        area = proper[region_id].area
        if score < 0.15 and (sub_region_aera/area>0.4 or sub_region_aera>150):
            real_id = re_id+1
